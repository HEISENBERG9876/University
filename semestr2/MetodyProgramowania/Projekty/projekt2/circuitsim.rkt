...
;sim-add-action
(define (sim-add-action! sim delay action-f)
  (define event-queue (sim-event-queue sim))
  (define exec-time (+ (sim-current-time sim) delay))
...
(define (sim-wait! sim wait-time)
  ...
  (define (call-each) ;wywolujemy akcje z symulacji
  (if (= (heap-count action-heap) 0)
      ...
      (let*
        ...
        (begin
          (displayln (action-time action))
          (function)
          (heap-remove-min! action-heap)
          (call-each))
        (void)))))

  (begin (set-sim-current-time! sim sim-end-time)
  (call-each)))
...
;funkcja pomocnicza do wire-set!
(define (add-all-actions-to-sim! wire)
  (define sim (wire-sim wire))
  (define action-list (wire-actions wire))
  (define (helper action-list)
    (if (null? action-list)
        (void)
        (let* ([current-action (car action-list)]
          ...
          (begin
          (sim-add-action! sim delay function)(helper (cdr action-list))))))
  (helper action-list))
...
(define (gate-or input-wire1 input-wire2 output-wire)
  ...
  (define or-output-function
    (lambda ()
      (set-wire-val! output-wire (or val1 val2))))
  (begin
...
(define (gate-xor input-wire1 input-wire2 output-wire)
  ...
  (define xor-output-function
    (lambda ()
      (set-wire-val! output-wire (xor val1 val2))))
  (begin
...
(define (gate-nand input-wire1 input-wire2 output-wire)
  ...
  (define nand-output-function
    (lambda ()
      (set-wire-val! output-wire (nand val1 val2))))
  (begin
...
(define (gate-nor input-wire1 input-wire2 output-wire)
  ...
  (define nor-output-function
    (lambda ()
      (set-wire-val! output-wire (nor val1 val2))))
  (begin
