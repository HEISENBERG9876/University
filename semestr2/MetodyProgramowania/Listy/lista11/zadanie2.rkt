...
(define (subst [e : Exp] [x : Symbol] [a : Exp] [bound-vars : (Listof Symbol)]) : Exp
  (type-case Exp e
    [(numE n)
     (numE n)]
    [(trueE)
    ...
    [(falseE)
     (falseE)]
    [(opE o l r)
     (opE o (subst l x a bound-vars) (subst r x a bound-vars))]
    [(ifE b l r)
     (ifE (subst b x a bound-vars) (subst l x a bound-vars) (subst r x a bound-vars))]
    [(varE y) 
     (if (eq? x y) (generate-valid a bound-vars) (varE y))] ;musimy zmodyfikowac a tak, zeby nie zawieralo zmiennych zwiazanych wczesniej.
    [(letE y e1 e2)
     (let ([e1-new (subst e1 x a bound-vars)]
           [e2-new (if (eq? x y)
                       e2
                       (subst e2 x a (cons y bound-vars)))])
       (letE y e1-new e2-new))]))



;(subst (parse `{let s 1 w}) 'w (parse `s)) ;w e2 zamiast kazdego var musimy zwrocic (varE (gensym)) i ewentualnie sprawdzic, czy nazwa nie jest juz uzyta
(parse `{let s 1 w})
