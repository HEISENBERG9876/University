...
;;;;;;;;;
(define (fv [exp : Exp]) : (Listof Symbol)
  (type-case Exp exp
    [(numE n) '()]
    [(opE o l r) (append (fv l) (fv r))]
    [(ifE b l r) (append (fv b) (append (fv l) (fv r)))]
    [(varE x) (list x)]
    [(letE x e1 e2) (append (fv e1) (filter (lambda (y) (not (eq? x y))) (fv e2)))]
    [(lamE x b) (filter (lambda (y) (not (eq? x y))) (fv b))]
    [(appE e1 e2) (append (fv e1) (fv e2))]))


(define (find-unique xs)
  (local
    [(define (helper xs result)
       (cond
         [(empty? xs) result]
         [(member (first xs) result) (helper (rest xs) result)]
         [else (helper (rest xs) (cons (first xs) result))]
      ))]
  (helper xs '())))

(define (fv2 [exp : Exp]) : (Listof Symbol)
  (find-unique (fv exp)))


(fv2 (parse `{let x 1 {+ x (+ (+ z x) (+ (+ j j) x))}}))
(fv2 (parse `x))
