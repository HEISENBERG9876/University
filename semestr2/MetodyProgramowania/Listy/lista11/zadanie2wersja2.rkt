...
(define (gensym)
  ...
  (string->symbol (string-append "new-x" (to-string counter)))))
  ...
  (type-case Exp e
    ...
    [(varE y)
     (if (eq? x y)
         ...
         (varE y))]
    [(letE y e1 e2)
     (let* ([fresh-var (gensym)] ; Step 1: Generate a fresh variable name
            [a-updated (subst a x (varE fresh-var))]) ; Step 2: Update substitution expression
       (letE y (subst e1 x a) ; Perform substitution in e1
             (subst e2 x (subst e1 y (varE fresh-var)))))])) ; Step 3: Perform substitution in e2 with updated substitution expression

;musimy zamienic zmienne wolne w a ktore sa takie same jak zmienne wolne w e na gensym
(subst (parse `{let s 1 w}) 'w (parse `s))

;wynik tego dzialania zamiast zwracac
;(letE 's (numE 1) (varE 's))

;powinien zwracac
;(letE 's (numE 1) (varE 'z))

;gdzie z to swiezo wygenerowana zmienna
;do podstawienia dochodzi w momencie, gdy na potykamy zmienna. Musimy wtedy sprawdzic, czy nazwa

;musimy miec liste zwiazanych zmiennych w e1 i w a. W a zmieniamy nazwe kazdej zmiennej, ktora jest zwiazana w e1 i sprawdzamy, czy sie nie powtarza.
;musimy tylko sprawic, zeby w a nie bylo zmiennych takich jak w exp
