...
(define (parse-exp [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{try ANY ANY} s) ;NOWE
...
(define test3 (parse-exp `{try {cond {{= 2 1} 3} {{= 2 2 2} 8} {{= 2 2} 2}} 16}))
(eval test3)

(eval (parse-exp `{try {x 1 2} 4}))

(eval (parse-exp `{try {+ {error a "aaa"} 1} 4}))
