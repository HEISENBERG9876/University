...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{let* ANY ANY} s)
     (let*E (parse-bindings (s-exp->list (second (s-exp->list s))))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(let*E bindings body)
     (eval body (extend-env3 env bindings))]))


(define (extend-env2 [orig-env : Env] [bindings : (Listof (Symbol * Exp))] [env : Env]) : Env ;dziala na zwyklym let
  (if (empty? bindings)
      env
      (extend-env2 orig-env (rest bindings) (extend-env env (fst (first bindings)) (eval (snd (first bindings)) orig-env) ))))
...
(eval (parse `{let* {{x {+ 1 2}} {z 1}} {+ x 1}}) mt-env) ;dziala
(eval (parse `{let* {{x {+ 1 2}} {z x}} {+ x 1}}) mt-env) ;dziala
