...
(define-type Exp
  ...
  (varE [x : Symbol])
  (letE [bindings : (Listof (Symbol * Exp))] [body : Exp])) ;bylo (letE [x : Symbol] [e1 : Exp] [e2 : Exp])
  ...
  (cond
    ...
    [(s-exp-match? `{let ANY ANY} s)
     (letE (parse-bindings (s-exp->list (second (s-exp->list s))))
           (parse (third (s-exp->list s))))] ;let przesuniety tutaj
...
(define (parse-bindings [bindings : (Listof S-Exp)]) : (Listof (Symbol * Exp))
  (type-case (Listof S-Exp) bindings
    [empty
     empty]
    [(cons binding rst)
     (if (s-exp-match? `{SYMBOL ANY} binding)
         (cons (pair (s-exp->symbol (first (s-exp->list binding)))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE bindings body)
     (eval body (extend-env2 env bindings env))]))


(define (extend-env2 [orig-env : Env] [bindings : (Listof (Symbol * Exp))] [result : Env]) : Env
  (if (empty? bindings)
      result
      (extend-env2 orig-env (rest bindings) (extend-env result (fst (first bindings)) (eval (snd (first bindings)) orig-env) ))))
...
;(eval (parse `{let {{x {+ 1 2}} {z x}} {+ x 1}}) mt-env) ;blad lookup, dziala
