    ...
    [(cons s ss)
     (if (s-exp-match? `{ANY ANY} s)
         (cons (pair (parse-exp (first (s-exp->list s)))
                     (parse-exp (second (s-exp->list s))))
               (parse-cond ss))
         (list (pair (errorE 'parse-exp "cond error") (errorE 'parse-exp "cond error"))))]))
...
(define (parse-op [op : Symbol]) : Op
  (cond
    ...
    [(eq? op '=) (eql)]
    [(eq? op '<=) (leq)]
    [else (error 'parse-op "unknown operator")])) ;(errOP)
...
(define (op->proc [op : Op]) : (Value Value -> Answer)
  (type-case Op op
    ...
    [(leq) (op-num-bool->proc <=)])) ;(lambda(x y) (errorA)) tryOp
