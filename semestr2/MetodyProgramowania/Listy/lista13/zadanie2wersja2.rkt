...
(define (check-prime n prime-list)
  (if (null? prime-list)
      #t
      (let ([x (car prime-list)])
        (if (= 0 (modulo n x))
            #f
            (check-prime n (cdr prime-list))))))


(define (next-prime-generator n prime-list) ;n to nowa potencjalna liczba pierwsza, z zalozenia wieksza od kazdej liczby w prime-list.
  (if (check-prime n prime-list)
      (stream-cons n (next-prime-generator (+ n 1) (cons n prime-list)))
      (next-prime-generator (+ n 1) prime-list)))

(define prime-stream2
  (next-prime-generator 2 '()))
