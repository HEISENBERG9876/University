...
(define (extend-env [env : Env] [x : Symbol] [v : Promise]) : Env ;NOWE
  (cons (bind x v) env))

(define (lookup-env [n : Symbol] [env : Env]) : Promise ;lookup-env nadal zwraca value.
...
(define (apply [v1 : Value] [v2 : Exp]) : Value
  (type-case Value v1
    [(funV x b env)
     (eval b (extend-env env x (delay v2 env)))]
