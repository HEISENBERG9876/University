...
(define (update_board_state-env! [env : Env] [x : Symbol] [v : Value]) : Void
  ...
  (type-case Exp e
    ...
    [(letrecE x e1 e2)
     (let* ([new-env (extend-env-undef env x)]
            [v1 (eval e1 new-env)])
       (begin (update_board_state-env! new-env x v1)
              (eval e2 new-env)))]
    [(beginE e1 e2)
     (begin (eval e1 env)
            (eval e2 env))]
    [(setE x e0)
     (begin 
       (update_board_state-env! env x (eval e0 env))
       (voidV))]
    [(whileE c e)
     (eval-while c e (voidV) env)]
    ))

(define (eval-while [c : Exp] [e : Exp] [result : Value] [env : Env]) : Value
  (let ([cond-val (boolV-b (eval c env))]) ;jeszcze dodac type-case, w momencie gdy doliczymy sie do czegos innego niz boolV.
    (if cond-val
          (eval-while c e (eval e env) env)
        result)))
...
(run `{let x 1 {while {<= x 5} {begin {set! x {+ 1 x}} x}}})

(run `{let x 5 {while {= x 5} {begin {set! x {+ 1 x}} x}}})

(run `{let x 6 {while {= x 5} {begin {set! x {+ 1 x}} x}}})

(run `{let x 1 {while {<= x 5} x}})
