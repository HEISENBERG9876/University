...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (funV x b)] ;NOWE
    [(appE e1 e2)
     (apply (eval e1 env) (eval e2 env) env)])) ;NOWE

(define (apply [v1 : Value] [v2 : Value] [env : Env]) : Value ;NOWE
  (type-case Value v1
    [(funV x b) ;NOWE
...
;W 2. p, czyli lambda(x) (+ x a) oznacza lambda(x) (+ x 1), gdyż a nie jest nadpisywane przez dalsze wystąpienia dlatego,
;że przy ewaluacji lambdy uzywamy srodowiska, ktore bylo dostepne w jej definiowania.
...
;fact5 -> 5 * fact4 -> 5 * (4 * fact3) .... -> 120

;{fact 5} -> {lambda {n} {* n {lambda {n} {* n {lambda {n} {* n ...} 3} 4} 5}
