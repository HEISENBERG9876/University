...
(define (extend-env [env : Env] [x : Symbol] [v : (-> Value)]) : Env ;NOWE
  (cons (bind x v) env))

(define (lookup-env [n : Symbol] [env : Env]) : Value ;lookup-env nadal zwraca value.
  (type-case (Listof Binding) env
    [empty (error 'lookup "unbound variable")]
    [(cons b rst-env) (cond
                        [(eq? n (bind-name b))
                         ((bind-val b))] ;NOWE wywolujemy bind-val b, gdyz jest to odroczone obliczenie.
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) e2)])) ;NOWE

(define (apply [v1 : Value] [v2 : Exp]) : Value ;NOWE
  (type-case Value v1
    [(funV x b env)
     (eval b (extend-env env x (lambda() (eval v2 env))))] ;NOWE
    [else (error 'apply "not a function")]))

;przyklad {{lambda x {let z {+ z x x} z} {+ 1 2}}}
;gorliwy:
;wyrazenie w let obliczamy majac w srodowisku x = 3.

;leniwy:
;wyrazenie w let obliczamy majac w srodowisku x = (lambda() (eval (OpE (add) (numE 1) (numE 2)). Musimy taka funkcje wywolac za kazdym razem, gdy natrafimy na x.
