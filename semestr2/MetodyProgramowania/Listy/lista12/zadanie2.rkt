...
(define-type ExpA
  ...
  (varA [n : Number]) ;pozycja w srodowisku
  (letA [a1 : ExpA] [a2 : ExpA])
  (lamA [e : ExpA]) ;NOWE. Nie potrzebujemy symbolu.
...
;w normalnym srodowisku mamy pary [zmienna, wartosc]. W envA mamy tylko wartosci Value. W zmiennych mamy ich pozycje w srodowisku wzgledem wiazania let/lambda.
...
(define (evalA [a : ExpA] [env : (EnvA Value)]) : Value
  (type-case ExpA a
    ...
    [(ifA b l r)
     (type-case Value (evalA b env)
       ...
       [else
        (error 'eval "type error")])]
    [(varA n)
     (lookup-envA n env)] ;patrzymy na pozycje w srodowisku, dostajemy Value
    [(letA e1 e2)
     (let ([v1 (evalA e1 env)])
    ...
    [(lamA b)
     (funAV b env)]
    [(appA e1 e2)
     (apply2 (evalA e1 env) (evalA e2 env))]))


(define (apply2 [v1 : Value] [v2 : Value]) : Value
  (type-case Value v1
    [(funAV b env)
...
(module+ test
  ...
  (test (run `{let z 1
        ...
        (runA `{let x 1
                 {+ x {let x {+ x 1}
                        {* x 3}}}}))
  (test (run `{{lambda {x} {{lambda {y} {+ x y}} 4}} 5})
        (runA `{{lambda {x} {{lambda {y} {+ x y}} 4}} 5}))
  ;(test (run `{lambda {x} {lambda {y} {+ x y}}}) to nie zadziala, bo mamy typ funV i funAV. Dopiero przy aplikacji wyniki sa takie same.
      ;  (runA `{lambda {x} {lambda {y} {+ x y}}}))
  )

;(define oryg (parse `{{lambda {x} {let y {* x x} {+ x y}}} 5}))
;(define translated (translate oryg mt-env))

;(eval oryg mt-env)
;(evalA translated mt-env)

;reprezentacja indeksowa dobrze wspolgra z domknieciami dlatego, ze w przypadku domkniec, srodowisko jest juz
;okreslone w momencie, w ktorym definiujemy let'a czy lambde i dalsze wystapienia konstrukcji wiazacych nie maja
;wplywu na domkniecia funkcji, ktore sa juz w srodowisku. Dlatego dodanie zmiennej o tej samej nazwie do srodowiska
;nie zmieni znaczenia wczesniejszych wystapien tej samej zmiennej.
...
;powinno zwrocic 1 zamiast 10 i tak tez sie dzieje zarowno zarowno w normalnej reprezentacji z domknieciami,
;jak i w przypadku reprezentacji leksykalnej.

(evalA
 (translate (parse `{let a 1
            {let p {lambda {x} {+ x a}}
              {let a 10
                {p 0}}}}) mt-env) mt-env)
...
;rozwoj srodowiska:
;[1] -> [(funAV (varA 0 + varA 1) [1]) , 1] -> [10 , (funAV (varA 0 + varA 1) [1]) , 1]
;nastepnie wykonujemy (apply (funAV (varA 0 + varA 1) [1]) 0) -> (eval (funAV (varA 0 + varA 1) [0, 1])) -> 1,
;czyli a = 10 nie jest widoczne podczas ewalucji naszego wyrazenia.