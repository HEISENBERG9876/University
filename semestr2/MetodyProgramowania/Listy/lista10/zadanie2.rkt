...
(define (parse-instr stack instr)
  (if (empty? instr)
      (first stack)
      (type-case Instr (first instr)
        [(pushI n) (parse-instr (cons (numE n) stack) (rest instr))]
        [(opI o) (parse-instr (cons (opE o (second stack) (first stack)) (rest (rest stack))) (rest instr))])))


(define (decompile instr)
  (parse-instr '() instr))

;;;;;;;;;;;;;;;;;;
(compile (parse `{+ {* 2 3} 5}))
(decompile (list (pushI 2) (pushI 3) (opI (mul)) (pushI 5) (opI (add))))
(compile (decompile (list (pushI 2) (pushI 3) (opI (mul)) (pushI 5) (opI (add)))))

(list (pushI 2) (pushI 3) (opI (mul)) (pushI 5) (opI (add)))
;;;;
...
(module+ test
  ...
  ;;;
  (test (compile (decompile (compile (parse `{+ {* 2 3} {+ 5 8}})))) (compile (parse `{+ {* 2 3} {+ 5 8}})) )
  (test (compile (decompile (compile (parse `{+ (- 2 5) (/ 1 (* 2 4))})))) (compile (parse `{+ (- 2 5) (/ 1 (* 2 4))})) )
  (test (compile (decompile (compile (parse `{+ {* (+ 1 (/ 6 (* 4 (- 68 10)))) 3} {+ 5 8}})))) (compile (parse `{+ {* (+ 1 (/ 6 (* 4 (- 68 10)))) 3} {+ 5 8}})) )


  )

(parse `{* (+ 1 (/ 2 3)) 4})
(compile (parse `{* (+ 1 (/ 2 3)) 4}))
(decompile (compile (parse `{* (+ 1 (/ 2 3)) 4})))
